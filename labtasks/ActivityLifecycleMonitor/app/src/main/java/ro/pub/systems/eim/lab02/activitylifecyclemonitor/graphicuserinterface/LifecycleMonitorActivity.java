...
public class LifecycleMonitorActivity extends AppCompatActivity {
    ...
    @Override
    protected void onRestoreInstanceState(Bundle restoredInstance) {
        super.onRestoreInstanceState(restoredInstance);

        if(restoredInstance.containsKey(Constants.USERNAME_EDIT_TEXT)) {
            EditText usernameEditText = (EditText) findViewById(R.id.username_edit_text);
            usernameEditText.setText(restoredInstance.getString(Constants.USERNAME_EDIT_TEXT));
        }

        if (restoredInstance.containsKey(Constants.PASSWORD_EDIT_TEXT)) {
            EditText passwordEditText = (EditText) findViewById(R.id.password_edit_text);
            passwordEditText.setText(restoredInstance.getString(Constants.PASSWORD_EDIT_TEXT));
        }

        if (restoredInstance.containsKey(Constants.REMEMBER_ME_CHECKBOX)) {
            CheckBox rememberMeCheckBox = (CheckBox) findViewById(R.id.remember_me_checkbox);
            rememberMeCheckBox.setChecked(restoredInstance.getBoolean(Constants.REMEMBER_ME_CHECKBOX));
        }
    }
}